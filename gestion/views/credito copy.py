...
class CreditoViewSet(viewsets.ModelViewSet):
    ...
    def buscar_por_fecha(self, request):
        ...
        # Modificar las cuotas en la respuesta para incluir solo las que cumplan la condición Q(fecha_pago=fecha, estado='activo')
        for credito in creditos_data:
            cuota_relevante = cuotas_filtradas.filter(
                credito_id=credito['id'], fecha_pago=fecha, estado='activo'
            ).first()
            if cuota_relevante:
                credito['cuota'] = {
